...
def moving_average(df: pd.DataFrame, price_col = 'Adj Close', windows = [20, 50, 200]) -> pd.DataFrame:
    for w in windows:
        df[f'MA_{w}'] = df['Adj Close'].rolling(window=w).mean()
    return df

def bolllinger_bands(df: pd.DataFrame, price_col = 'Adj Close', window: int = 20, num_std: int = 2) -> pd.DataFrame:
...
def macd(df, price_col='Adj Close', span_short=12, span_long=26, span_signal=9):
...
def rsi(df, price_col='Adj Close', period=14):
